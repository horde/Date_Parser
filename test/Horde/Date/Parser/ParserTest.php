...
class ParserTest extends Horde_Test_Case
{
    ...
    public function testPostNormalizeAmPmAliases()
    {
        ...
        $tokens = [new Horde_Date_Parser_Token('5:00'), new Horde_Date_Parser_Token('morning')];
        ...
        $tokens = $parser->dealiasAndDisambiguateTimes($tokens, []);

        $this->assertEquals('am', $tokens[1]->tags[0][1]->type);
        $this->assertEquals(2, count($tokens));

        // don't affect unwanted patterns
        $tokens = [new Horde_Date_Parser_Token('friday'), new Horde_Date_Parser_Token('morning')];
